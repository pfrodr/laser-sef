...
for pasta in pastas:
    ...
    with open(pasta_processar + "/INDICE.xml", encoding="ISO-8859-1") as xml:
        ...
        for f in files:
            if not f.endswith(".xml") and not f.endswith(".zip"):
                ...
                if qtd_arquivos_pasta < qtd_max_pasta:
                    shutil.copyfile(os.path.join(pasta_processar,f), os.path.join(pasta_atual,f))
                elif qtd_arquivos_pasta == qtd_max_pasta:
                    with open(pasta_atual + "/INDICE.xml",'w', encoding="ISO-8859-1") as indice:
                        indice.write(xml_header[0] + "".join(xml_items[0:qtd_arquivos_pasta]) + xml_final)
        ...
        with open(pasta_atual + "/INDICE.xml",'w', encoding="ISO-8859-1") as indice:
            indice.write(xml_header[0] + "".join(xml_items[0:fun.qtd_arquivos(pasta_atual)]) + xml_final)
